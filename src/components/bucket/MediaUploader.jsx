...
const MediaUploader = ({ onUpload, bucketId, onClose }) => {
    ...
    const handleUpload = async () => {
        ...
        if (!files.length || !bucketId) {
            alert("Please select files and enter a bucket ID");
            return;
        }
        ...
        for (let i = 0; i < files.length; i++) {
            formData.append("files", files[i]); // 'files' field to match backend
        }
    ...
    return (
        <motion.div
            className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 w-full h-[calc(100vh-10rem)] overflow-y-scroll"
                    ...
                    <AnimatePresence>
                        {files.length > 0 && (
                            ...
                            >
                                ...
                                <div className="space-y-2 max-h-60 overflow-y-scroll pr-2 custom-scrollbar">
