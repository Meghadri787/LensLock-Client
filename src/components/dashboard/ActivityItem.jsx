...
const ActivityItem = ({ data, name, bucketId, delay = 0 }) => {
    const { manageAccessRequest, isLoading } = useBucketStore();
    ...
    return (
        ...
        >
            <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                    <p className="text-base font-semibold text-gray-900">
                        {user.name} requested bucket "{name}"
                    </p>
            ...
            <div className="flex gap-2">
                <button
                    onClick={() =>
                        handleRequestManage(bucketId, user._id, "accept")
                    }
                    className="px-3 py-1.5 bg-green-50 text-green-600 rounded-md text-sm font-medium hover:bg-green-100 transition-colors"
                >
                    {isLoading ? (
                ...
                <button
                    onClick={() =>
                        handleRequestManage(bucketId, user._id, "reject")
                    }
